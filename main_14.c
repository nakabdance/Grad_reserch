...
double Info_qt[2][5][5];
int usefuleness = 0;
int next_usefuleness;
double Act_qt[4][5];
int Wall_judge;
double *rewards;
...
void Info(Agent *agent, int);
void Info_Learn(Agent *agent, int);
...
*/
			...
			Info(&agent, steps);
			/*
			int i = agent.sight_g1 - 1;
			int s = agent.sight_g2 - 1;

			for (int j = 0; j < 2; j++)
			{
				printf("info num q : %f ", Info_qt[j][i][s]);
			}
			printf("\n");
*/
			Act(&agent);
			...
			*/
			...
			//１学習の中でどれだけの報酬を獲得したのかを保持する
			rewards_post(&agent, goal_steps(&agent, map_g1, map_g2, steps), steps);
			...
			for (int j = 0; j < 2; j++)
			{
				printf("info teach q : %f ", Info_qt[j][i][s]);
			}
			printf("\n");

			for (int j = 0; j < 4; j++)
			{
				printf("act teach q : %f ", Act_qt[j][p]);
			}
			printf("\n");
			*/
...
void agent_sight_g1(Agent *agent)
{
	...
	(*agent).sight_g1 = map_g1[(*agent).location_x][(*agent).location_y];
}
...
void agent_sight_g2(Agent *agent)
{
	...
	(*agent).sight_g2 = 0;
	(*agent).sight_g2 = map_g2[(*agent).location_x][(*agent).location_y];
}

void next_agent_sight_g1(Agent *agent)
{

	(*agent).next_sight_g1 = 0;

	(*agent).next_sight_g1 = map_g1[(*agent).location_x][(*agent).location_y];
}

void next_agent_sight_g2(Agent *agent)
{

	(*agent).next_sight_g2 = 0;

	(*agent).next_sight_g2 = map_g2[(*agent).location_x][(*agent).location_y];
}
...
void rewards_post(Agent *agent, double c, int isteps)
{
...
void Infoselect(int isteps, Agent *agent, int ilearn2)
{
	...
	if (genrand_real1() < epsilon)
	{
		...
		if (i == 0)
		{
			info_choice = (*agent).sight_g1;
		}
		if (i == 1)
		{
			info_choice = (*agent).sight_g2;
		}
	}
	else
	{
...
void choice_NextQMAX(Agent *agent)
{
	...
	{
		i = next_y[0];
		next_info_choice = (*agent).next_sight_g1;
	}
	else
	{
		i = next_y[1];
		next_info_choice = (*agent).next_sight_g2;
	}
	next_maxq = i;
}
...
void Info(Agent *agent, int isteps)
{
	int i, j, max_q, s;
	double num_q[2];
	...
	else
	{
		...
		else if (num_q[0] == num_q[1])
		{
			usefuleness = genrand_int32() % 2;
			if (usefuleness == 0)
			{
				Choice_info = (*agent).sight_g1;
			}
...
void Info_Learn(Agent *agent, int isteps)
{
	int i, j, max_q, s;
	double xnum_q[2];
	...
	double alpha = ALPHA;
	double gamma = GAMMA;
	double q = Info_qt[usefuleness][(*agent).sight_g1 - 1][(*agent).sight_g2 - 1];

	i = (*agent).next_sight_g1 - 1;
	s = (*agent).next_sight_g2 - 1;

	for (j = 0; j < 2; j++)
	{
		xnum_q[j] = Info_qt[j][i][s];
	}

	if (xnum_q[0] > xnum_q[1])
	{
		next_usefuleness = 0;
	}
	else if (xnum_q[0] == xnum_q[1])
	{
		next_usefuleness = genrand_int32() % 2;
	}
	else if (xnum_q[0] < xnum_q[1])
	{
		next_usefuleness = 1;
	...
	Info_qt[usefuleness][(*agent).sight_g1 - 1][(*agent).sight_g2 - 1] = q + alpha * ((double)rewards[isteps] + (gamma * xnum_q[next_usefuleness]) - q);
}
...
void Act_Learn(Agent *agent, int isteps)
{
	int i, j, max_q, n, m;
	int num_q[4];
	int same_q[4];
	...
	double alpha = ALPHA;
	double gamma = GAMMA;
	double q = Act_qt[(*agent).agent_action_select][Choice_info - 1];
	...
	Act_qt[(*agent).agent_action_select][Choice_info - 1] = q + alpha * ((double)rewards[isteps] + (gamma * num_q[m]) - q);
}

void ReadData(Agent *agent, int isteps)
{
	int sight_g1, sight_g2;
	sight_g1 = (*agent).sight_g1;
	sight_g2 = (*agent).sight_g2;
...
{
	int sight_g1, sight_g2;
	sight_g1 = (*agent).next_sight_g1;
	sight_g2 = (*agent).next_sight_g2;
